...
void VertexArray::AddBuffer(const VertexBuffer& vb, const VertexBufferLayout& layout)
{
	...
	for (unsigned int i = 0; i < elements.size(); i++)
	{
		...
		GLCall(glVertexAttribPointer(i, element.m_count, element.m_type, element.m_normalized, layout.GetStride(), (const void*)offset)); // 第一个0为顶点属性中位置属性的索引，在着色器中被指定   参数2为一个属性所占元素数量， 2 * sizeof(float)为一整组属性所占字节数， 最后的0是此属性的起始偏移
